...
int main()
{
  int x,key=0,numoflines=0,numofd=0,*diarytag,requesteddiary=0;
  
    diarytag=(int *)malloc(sizeof(int));
  char ***text;
  printf("1-adding a diary   2-showing a diary  3-ending the app\n");
  ...
  while( key != 3 ){
  	...
  	if( key == 2 ){
  		...
  		scanf("%d",&requesteddiary);
  		 for ( x=0;x<diarytag[requesteddiary-1];x++){
